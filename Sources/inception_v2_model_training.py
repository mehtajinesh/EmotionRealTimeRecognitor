"""
File name: inception_v2_model_training.py
Author: Jinesh Mehta
File Description: This file contains the functions to train the model.
"""
import os
from constants import INCEPTION_V2_MODEL_PATH, INCEPTION_FER_IMG_HEIGHT, INCEPTION_FER_IMG_WIDTH,\
...
def fine_tune_from_resnet(model, train_generator, train_data_x, val_data):
    ...
    check_point = ModelCheckpoint(
        filepath=os.path.join(CHECKPOINT_DIRECTORY,
                              'Inception-ResNet-v2_{epoch:02d}_{val_loss:.2f}.h5'),
        monitor='val_loss',
        save_best_only=True,
        mode='auto',
        period=1)
...
def main():
    ...
    save_model(trained_model, INCEPTION_V2_MODEL_PATH)
